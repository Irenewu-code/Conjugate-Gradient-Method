...
listx = ["2", '4', '10', '20', '40', '50', '60', '80', '100']
# listx =["50"]
for i in range(len(listx)):
    ...
    # print(x_CG)
    Req = r_equal(g, x_CG[2], N, V)  # equivant resistance in this network
    t0 = (time.time() - t0)           # calculate CPU time
    print('CG CPU time = {:g} seconds'.format(t0)) # print out CPU time
    
    # print("accuracy of LU: ", norm2( A @ x_LU - b) )
    # print("accuracy of CG: ", norm2( A @ x_CG[2] - b) )
    # print('accuracy of CG vs. LU: ', norm2( x_LU - x_CG[2]) )
    print("Iter :", x_CG[1])
    print("Vw", get_Vw(N,x_CG[2]))
    print("Vne", get_Vne(N,x_CG[2]))
    print("Ve", get_Ve(N,x_CG[2]))
    Req = round(Req, 2)         # Round to the second decimal place
    print(f'Req = {Req} Ohms')  # print out Req
